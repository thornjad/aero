;;; coffeescript-mode.el --- Major mode for CoffeeScript -*- lexical-binding: t; -*-
...
;; URL: http://gitlab.com/thornjad/coffeescript-mode
...
(defconst coffeescript-mode-version "0.9.0"
  "The version of `coffeescript-mode'.")
...
(defcustom coffeescript-mode-hook nil
  "Hook called by `coffeescript-mode'.  Examples:
...
(defun coffee-start-compile-process (curbuf line column)
  (lambda (start end)
    (let ((proc (apply 'start-file-process "coffeescript-mode"
...
(defun coffee-version ()
  "Show the `coffeescript-mode' version in the echo area."
  (interactive)
  (message (concat "coffeescript-mode version " coffeescript-mode-version)))
...
(easy-menu-define coffeescript-mode-menu coffeescript-mode-map
...
(defvar coffee-js-keywords
  '("if" "else" "new" "return" "try" "catch"
    ...
    "async" "await" "class" "until" "loop" "yield"))
...
(defvar coffeescript-mode-syntax-table
...
;;;###autoload
(define-derived-mode coffeescript-mode prog-mode "Coffee"
  ...
  ;; Don't let electric-indent-mode break coffeescript-mode.
...
(define-minor-mode coffee-cos-mode
  "Toggle compile-on-save for coffeescript-mode.

Add `'(lambda () (coffee-cos-mode t))' to `coffeescript-mode-hook' to turn
it on by default."
...
(provide 'coffeescript-mode)

;;
;; On Load
;;

;; Run coffeescript-mode for files ending in .coffee.
;;;###autoload
(add-to-list 'auto-mode-alist '("\\.coffee\\'" . coffeescript-mode))
;;;###autoload
(add-to-list 'auto-mode-alist '("\\.iced\\'" . coffeescript-mode))
;;;###autoload
(add-to-list 'auto-mode-alist '("Cakefile\\'" . coffeescript-mode))
;;;###autoload
(add-to-list 'auto-mode-alist '("\\.cson\\'" . coffeescript-mode))
;;;###autoload
(add-to-list 'interpreter-mode-alist '("coffee" . coffeescript-mode))

;;; coffeescript-mode.el ends here
